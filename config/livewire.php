...
return [
    ...
    */
    ...
    | Component Layout
    |---------------------------------------------------------------------------
    | The default layout for components that render as a full page.
    |
    | CHANGED FROM 'layouts::app'. The default expects a view namespace registered
    | as `layouts`, which Livewire's own publish command creates. This project
    | uses conventional anonymous Blade components under resources/views/components,
    | which Laravel resolves by DOT NOTATION — so `layouts::app` fails with
    | "No hint path defined for [layouts]" and every full-page component 500s.
    |
    | Individual components still declare their own layout with the #[Layout]
    | attribute; this is only the fallback for one that does not.
    */
    ...
    */
    ...
    |
    | CHANGED FROM THE DEFAULT ('sfc') TO 'class'.
    |
    | Livewire 4 defaults to single-file components, which are excellent for small
    | presentational pieces. This application is not that: the request wizard,
    | the approval queue and the governance workspace all carry workflow logic that
    | the vendor brief requires to be tested transition by transition (§7.2), and
    | logic buried in a Blade template is materially harder to test than a class.
    |
    | Class-based components also keep the emoji prefix out of file names, which
    | matters when a path is quoted in a deployment script — the ⚡ character has
    | caused encoding trouble in shell commands before.
    |
    */
